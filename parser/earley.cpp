...
#define add_item(i, j) outs[i].emplace(j), ins[j].emplace(i), front.emplace(j), front.erase(i)
start:	wcout << format(G, i) << endl;
	assert(i.alt < g.size() && i.dot <= g[i.alt].size());
	if (g[i.alt].size() <= i.dot) {
		if (i.nt != UINT_MAX)
		for (auto it = outs.lower_bound(eitem(i.end - i.len, i.nt, 0, 0, 0));
