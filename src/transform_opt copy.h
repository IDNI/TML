 ...
 * the command pattern.
 */
typedef std::function<bool(struct mutated_prog&)> mutation;
...
struct my_deleter {
    my_deleter() {};
    my_deleter(const my_deleter&) { std::cout << "D copy ctor\n"; }
    my_deleter(my_deleter&) { std::cout << "D non-const copy ctor\n";}
    my_deleter(my_deleter&&) { std::cout << "D move ctor \n"; }
    void operator()(mutation* p) const {
