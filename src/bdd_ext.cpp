...
spbdd_handle bdd_mult_dfs(cr_spbdd_handle x, cr_spbdd_handle y, spbdd_handle *z, size_t bits, size_t n_vars ) {
	...
	return bdd_handle::get(bdd::MULT_DFS(x->b,y->b, z_aux , 0, bits, n_vars, true));
}
...
int_t bdd::ADDER_IDX(int_t a_in, int_t b_in, bool carry, size_t bit, size_t depth, size_t n_args) {
	...
	if (a_in == T && b_in == T)
		...
		if (carry == false)
			c = add(pos,
					bdd_or(ADDER_IDX(a.h, b.l, false, bit+1, depth, n_args), ADDER_IDX(a.l, b.h, false, bit+1, depth, n_args)),
					bdd_or(ADDER_IDX(a.h, b.h, true, bit+1, depth, n_args ), ADDER_IDX(a.l, b.l, false, bit+1, depth, n_args)));
		else
			c = add(pos,
					bdd_or(ADDER_IDX(a.h, b.h, true, bit+1, depth, n_args), ADDER_IDX(a.l, b.l, false, bit+1, depth, n_args)),
					bdd_or(ADDER_IDX(a.h, b.l, true, bit+1, depth, n_args), ADDER_IDX(a.l, b.h, true, bit+1, depth, n_args )));
	}
	return c;
...
int_t bdd::MULT_DFS(int_t a_in, int_t b_in, int_t *accs, size_t depth, size_t bits, size_t n_args, bool bit_acc) {

	int_t c;
	uint_t pos = 4 + n_args * depth;
