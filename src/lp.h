// LICENSE
// This software is free for use and redistribution while including this
// license notice, unless:
// 1. is used for commercial or non-personal purposes, or
// 2. used for a product which includes or associated with a blockchain or other
// decentralized database technology, or
// 3. used for a product which includes or associated with the issuance or use
// of cryptographic or electronic currencies/coins/tokens.
// On all of the mentioned cases, an explicit and written permission is required
// from the Author (Ohad Asor).
// Contact ohad@idni.org for requesting a permission. This license may be
// modified over time by the Author.
#ifndef __LP_H__
#define __LP_H__
#include <cstdint>
#include <vector>
#include <iostream>
#include <set>
#include <map>
#include "bdd.h"
#include "query.h"
#include "term.h"

void tml_init();

typedef std::map<prefix, spbdd> db_t;

struct prog_data {
	strs_t strs;
	std::unordered_map<int_t, term> strtrees;
	std::vector<term> out;
	matpairs r;
	matrix goals, tgoals;
	bool bwd = false;
	void clear() {
		strtrees.clear(), out.clear(), r.clear(), goals.clear(),
		tgoals.clear(), bwd = false;
	}
};

class lp { // [pfp] logic program
	friend struct rule;
	DBG(friend class driver;)
	std::vector<struct rule*> rules;
	void add_fact(spbdd f, const prefix& p);
	bool add_not_fact(spbdd f, const prefix& p);
	bool add_facts(const matrix& x);
	bool add_fact(const term& t);
	std::set<db_t::const_iterator> tree_prefix(const prefix& p) const;
	void get_tree(const prefix& p, spbdd root, db_t& out,
		std::set<spbdd>& done);
	void get_trees(const db_t& in, db_t& out);
	void get_trees();
	void fwd(db_t &add, db_t &del);
	const strs_t strs;
	db_t trees, gbdd;
	std::map<int_t, db_t> strtrees;
public:
	prog_data pd;
	range rng;
	db_t db;
	db_t trees_out;
	std::map<int_t, db_t> strtrees_out;
	lp(prog_data, range rng, lp *prev = 0);
	bool pfp();
	~lp();
};

extern int_t null;
#endif
